...
export async function requestEnrollment(courseId, access) {
  try {
    const response = await fetchFromAPI(
      `cursos/${courseId}/solicitar_inscripcion/`,
      access,
      {
        method: "POST",
      },
...
export async function checkEnrollment(courseId, access) {
  try {
    const response = await fetchFromAPI(
      `cursos/${courseId}/estado_inscripcion/`,
      access,
    );
...
export async function marcarLeccionCompletada(leccionId, access) {
  return await fetchFromAPI(
    `lecciones/${leccionId}/marcar_completada/`,
    access,
    {
      method: "POST",
    },
...
export async function registrarProgresoLeccion(leccionId, access, progreso) {
  return await fetchFromAPI(
    `lecciones/${leccionId}/registrar-progreso/`,
    access,
    {
      method: "POST",
      body: { progreso },
...
export async function agregarFeedbackLeccion(
  leccionId,
  comentario,
  calificacion,
  access,
  ...
  return await fetchFromAPI(
    `lecciones/${leccionId}/agregar-feedback/`,
    access,
    {
      method: "POST",
...
export async function getLeccionesCompletadas(cursoId, access) {
  try {
    const response = await fetchFromAPI(
      `cursos/${cursoId}/lecciones-completadas/`,
      access,
    );
    return response;
  } catch (error) {
...
export async function getEstadisticasInstructor(instructorId, access) {
  return await fetchFromAPI(
    `instructores/${instructorId}/estadisticas/`,
    access,
  );
}
...
export async function getCursosMatriculados(userId, token) {
  try {
    const response = await fetch(
      `${API_BASE_URL}/usuarios/${userId}/cursos-matriculados/`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
export async function getProgresoCompletoPorCurso(cursoId, userId, token) {
  const response = await fetch(
    `${API_BASE_URL}/cursos/${cursoId}/progreso-completo/${userId}/`,
    {
      headers: {
        Authorization: `Bearer ${token}`,
      },
